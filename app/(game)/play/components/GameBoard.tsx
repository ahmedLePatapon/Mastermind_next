...
export default function GameBoard({
    ...
    const [selectedPeg, setSelectedPeg] = useState<{ rowIndex: number; pegIndex: number } | null>(null);

    const handlePegClick = (rowIndex: number, pegIndex: number) => {
        setSelectedPeg((prev) =>
            prev && prev.rowIndex === rowIndex && prev.pegIndex === pegIndex
                ? null // Si on reclique sur le même peg → désélection
                : { rowIndex, pegIndex } // Sinon on sélectionne le nouveau peg
        );
    };
    ...
    return (
        <div className="bg-[#1a1a3a] p-4 rounded-xl space-y-3 justify-center">
            <p className="text-lg font-bold text-white" > Debug: {status}</p>
            {
                lines.length > 0 && (status === "playing") && lines.map((row, rowIdx) => (

                    <div
                        key={rowIdx}
                        className={`flex items-center justify-between p-3 rounded-lg ${row.guess.every((p) => !p) ? "bg-[#242447]/50 opacity-50" : "bg-[#242447]"
                            }`}
                    >
                            ...
                            <div className="flex gap-6">
                                {row?.guess.map((color, colorIdx) => (
                                    <Peg
                                        key={colorIdx}
                                        color={color ?? null}
                                        isEmpty={!color || color === "#6b7280"}
                                        isSelected={
                                            selectedPeg?.rowIndex === rowIdx &&
                                            selectedPeg?.pegIndex === colorIdx
                                        }
                                        dashed={false}
                                        shouldReset={
                                        ...
                                        onClick={() => handlePegClick(rowIdx, colorIdx)}
                                    />
                                ))}
